...
class TestCompareRAGUseCaseExecute:
    ...
    @pytest.mark.asyncio
    async def test_execute_creates_query_embedding(
        self, use_case, mock_dependencies
    ):
        ...
        mock_dependencies["embedding_service"].create_embeddings.assert_called_once_with(
            ["What is machine learning?"]
        )

    @pytest.mark.asyncio
    async def test_execute_searches_vector_repo_with_embedding(
        self, use_case, mock_dependencies
    ):
        ...
        mock_dependencies["vector_repo"].search.assert_called_once_with(
            [0.1, 0.2, 0.3], top_k=5
        )

    @pytest.mark.asyncio
    async def test_execute_searches_graph_repo_with_query(
        self, use_case, mock_dependencies
    ):
        ...
        mock_dependencies["graph_repo"].search.assert_called_once_with(
            "What is machine learning?"
        )

    @pytest.mark.asyncio
    async def test_execute_parallel_search(
        self, use_case, mock_dependencies
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_aggregates_vector_results(
    ...
    @pytest.mark.asyncio
    async def test_execute_aggregates_graph_results(
    ...
    @pytest.mark.asyncio
    async def test_execute_with_empty_vector_results(
        self, use_case, mock_dependencies
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_with_empty_graph_results(
        self, use_case, mock_dependencies
    ):
    ...
    async def test_execute_handles_vector_repo_error_gracefully(
        ...
        mock_dependencies["vector_repo"].search.side_effect = Exception(
    ...
    async def test_execute_handles_graph_repo_error_gracefully(
        ...
        mock_dependencies["vector_repo"].search.return_value = sample_vector_results
        mock_dependencies["graph_repo"].search.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_execute_with_top_k_parameter(
        self, use_case, mock_dependencies
    ):
        ...
        mock_dependencies["vector_repo"].search.assert_called_once_with(
            [0.1, 0.2, 0.3], top_k=10
        )

    @pytest.mark.asyncio
    async def test_comparison_result_has_correct_query(
        self, use_case, mock_dependencies
    ):
